...
typedef struct {
    ...
    GtkWidget *button[21]; // Declare an array of pointers to GtkWidget objects for buttons
} calc;
...
// Define flags for trigonometric functions
bool sin_flag = false;
bool cos_flag = false;
bool tan_flag = false;
bool cot_flag = false;
...
static void calculate(GtkButton *button, gpointer data) {
    ...
    if ((strcmp("+", text) == 0) || (strcmp("-", text) == 0) || (strcmp("/", text) == 0) || (strcmp("x", text) == 0) || (strcmp("=", text) == 0)) {
        ...
        // Set the trigonometric function flag based on the clicked button label
    ...
    if (strcmp("=", text) == 0) {
        ...
        // Check if any of the trigonometric function flags are set
        if (sin_flag) {
            double value = atof(input_buffer);
            result = sin(value);
        } else if (cos_flag) {
            double value = atof(input_buffer);
            result = cos(value);
        ...
        // Reset all the operation flags and trigonometric function flags
        add = false;
        mul = false;
        divv = false;
        sub = false;
...
static void activate(GtkApplication *app, gpointer user_data) {
    ...
    // Create the trigonometric function buttons
    ...
    widget.button[20] = gtk_button_new_with_label("cot");
    ...
    // Attach the trigonometric function buttons to the grid layout
    ...
    // Connect widgets buttons to their respective callback functions
    ...
    // Connect the trigonometric function buttons to their respective callback functions
    g_signal_connect(widget.button[17], "clicked", G_CALLBACK(calculate_sin), NULL);
    g_signal_connect(widget.button[18], "clicked", G_CALLBACK(calculate_cos), NULL);
    g_signal_connect(widget.button[19], "clicked", G_CALLBACK(calculate_tan), NULL);
    g_signal_connect(widget.button[20], "clicked", G_CALLBACK(calculate_cot), NULL);
